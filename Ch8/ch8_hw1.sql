...
SELECT pls14.stabr,
      ...
      sum(pls09.visits) AS visits_2009,
      round( (CAST(sum(pls14.visits) AS decimal(10, 1)) - sum(pls09.visits)) /
                    sum(pls09.visits) * 100, 2 ) AS visits_pct_change,
      round( (CAST(sum(pls14.gpterms) AS decimal(10, 1)) - sum(pls09.gpterms)) /
                    sum(pls09.gpterms) * 100, 2 ) AS gpterms_pct_change,
      round( (CAST(sum(pls14.pitusr) AS decimal(10, 1)) - sum(pls09.pitusr)) /
                    sum(pls09.pitusr) * 100, 2 ) AS pitusr_pct_change
FROM pls_fy2014_pupld14a pls14 JOIN pls_fy2009_pupld09a pls09  
ON pls14.fscskey = pls09.fscskey
WHERE pls14.visits >= 0 AND pls09.visits >= 0
    AND pls14.gpterms >= 0 AND pls09.gpterms >= 0
    AND pls14.pitusr >= 0 AND pls09.pitusr >= 0
GROUP BY pls14.stabr                    
ORDER BY gpterms_pct_change DESC;               


-- 2)
SELECT pls14.obereg,    -- Can't have stabr anymore since grouped
      sum(pls14.visits) AS visits_2014,     
      sum(pls09.visits) AS visits_2009,
      round( (CAST(sum(pls14.visits) AS decimal(10, 1)) - sum(pls09.visits)) /
                    sum(pls09.visits) * 100, 2 ) AS pct_change
...
-- 3)
SELECT pls14.libname, pls14.city, pls14.stabr, pls14.statstru, pls14.c_admin,
        pls14.branlib,
       pls09.libname, pls09.city, pls09.stabr, pls09.statstru, pls09.c_admin,
        pls09.branlib
...
-- Need to use FULL OUTER JOIN to display all rows from both tables
-- Got wrong. Question didn't state what var. is agencies, but it's comb.
-- ANS: To check NULLs in both tables, use PK.
...
ON pls14.fscskey = pls09.fscskey
WHERE pls14.fscskey IS NULL OR pls09.fscskey IS NULL; 
-- Ans: 236 Nulls from both tables
